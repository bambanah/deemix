...
class GW {
  ...
  async get_album_tracks (alb_id) {
    const tracks_array = []
    const body = await this.api_call('song.getListByAlbum', { ALB_ID: alb_id, nb: -1 })
  ...
  async get_artist_top_tracks (art_id, options = {}) {
    ...
    const tracks_array = []
    const body = await this.api_call('artist.getTopTrack', { ART_ID: art_id, nb: limit })
    body.data.forEach(track => {
  ...
  async get_playlist_tracks (playlist_id) {
    const tracks_array = []
    const body = await this.api_call('playlist.getSongs', { PLAYLIST_ID: playlist_id, nb: -1 })
    body.data.forEach(track => {
      track.POSITION = body.data.indexOf(track)
      tracks_array.push(track)
    })
  ...
  async get_track_with_fallback (sng_id) {
    ...
    if (parseInt(sng_id) > 0) {
      try { body = await this.get_track_page(sng_id) } catch (e) { /* nothing */ }
    }
  ...
  async get_user_playlists (user_id, options = {}) {
    const limit = options.limit || 25
    const user_profile_page = await this.get_user_profile_page(user_id, 'playlists', { limit })
