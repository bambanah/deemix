...
export default defineComponent({
	...
	setup(_, ctx) {
		...
		if (
			searchedTerm.value &&
			(!isMainSearchCached.value || isNewSearch.value)
		) {
	...
	methods: {
		...
		getViewInfo() {
			...
			return standardizeData(
				this.results[this.currentTab.viewInfo],
				this.currentTab.formatFunc
			);
		},
		...
		scrolledSearch() {
			...
			const needToPerformScrolledSearch =
				this.results[currentTabKey].next < this.results[currentTabKey].total;
		...
		isTabLoaded(tab) {
			return (
				this.loadedTabs.includes(tab.searchType) || tab.searchType === "all"
			);
		},
	},
});
