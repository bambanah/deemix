const ru = {
  globals: {
    welcome: "Добро пожаловать в DeemixFix",
    back: "назад",
    loading: "загрузка",
    ...
    listTabs: {
      ...
      spotifyPlaylist:
        "плейлист spotify | плейлисты spotify | плейлисты spotify",
  ...
  about: {
    ...
    updates: {
      ...
      updateAvailable:
        "Вы используете не последнюю доступную версию: {version}",
    ...
    subtitles: {
      ...
      contributing:
        "Хотите внести вклад в развитие этого проекта? Это можно сделать разными способами!",
    ...
    thanks:
      "Спасибо <strong>rtonno</strong>, <strong>uhwot</strong> и <strong>lollilol</strong> за помощь с этим проектом, а также <strong>BasCurtiz</strong> за иконку.",
    upToDate: {
    ...
    questions: {
      text: "Если у вас возникли вопросы или проблемы с приложением, поищите решение на {telegram}. Если не нашли решение, можете создать новый пост и описать вашу проблему.",
      telegram: "Telegram",
    },
    beforeReporting:
      "Перед тем, как сообщать об ошибках, убедитесь, что вы используете последнюю версию приложения и что проблема не на вашей стороне.",
    beSure:
      "Убедитесь, что ошибка возникает и на других устройствах. Также <strong>НЕ</strong> сообщайте об ошибке, если про неё уже известно.",
    duplicateReports:
      "Повторные сообщения об ошибках рассматриваться не будут.",
    dontOpenIssues:
      "<strong>НЕ</strong> используйте репозиторий для вопросов автору, для этого есть сабреддит.",
    newUI: {
    ...
    acceptFeatures:
      "Можете предложить новые функции, но не слишком сложные, так как они будут добавлены в приложение, а не в саму библиотеку.",
    otherLanguages:
      "Если вы хорошо знаете другой язык программирования, можете портировать на нём deemix!",
    understandingCode:
      "Не можете разобраться в коде? Свяжитесь с waLplanet в Telegram или на Reddit.",
    contributeWebUI: {
      text: "Если вы знаете Vue.js (JavaScript), HTML или CSS, можете внести вклад в развитие {webui}.",
      webui: "WebUI",
    },
    itsFree:
      "Помните, что это <strong>бесплатное приложение</strong> и вам следует <strong>поддерживать понравившихся исполнителей</strong> прежде, чем поддерживать разработчиков.",
  ...
  errors: {
    ...
    ids: {
      ...
      notYourPrivatePlaylist:
        "Вы не можете скачивать чужие приватные плейлисты.",
      ...
      wrongBitrateNoAlternative:
        "Данного трека нет в нужном битрейте. Альтернатив не найдено!",
      ...
      notAvailableNoAlternative:
        "Трек недоступен на серверах Deezer. Альтернатив не найдено!",
      ...
      wrongGeolocation:
        "Ваш аккаунт не может играть данный трек из вашей текущей страны.",
  ...
  home: {
    needTologin:
      "Вам необходимо войти под своей учетной записью Deezer, прежде чем вы сможете скачивать.",
  ...
  linkAnalyzer: {
    ...
    linkNotSupportedYet:
      "Эта ссылка не поддерживается, попробуйте вставить другую.",
    table: {
      id: "ID",
      isrc: "ISRC",
      upc: "UPC",
  ...
  settings: {
      ...
      arl: {
        ...
        howTo: {
          prologue: {
            ...
            warning:
              "Используйте этот метод, только если электронная почта и пароль не работают.",
          },
          chromeSteps: {
            title: "Chrome",
            ...
            easyWay: {
              ...
              step4:
                "Выберите «Файлы cookie» > deezer.com > Файлы cookie > arl",
              step5:
                "Выберите строку рядом с Контент, выделите ее всю и скопируйте",
            },
            step4:
              "Перейдите на вкладку «Application» (если вы ее не видите, нажмите двойную стрелку )",
            videoGuide: {
              text: "Вот {videoGuide}",
          ...
          firefoxSteps: {
            ...
            step4:
              "Перейдите на вкладку «Хранилище» (если вы ее не видите, нажмите двойную стрелку)",
          },
          commonSteps: {
            step2:
              "Перейдите на сайт www.deezer.com и войдите в свою учетную запись.",
            step3:
              "После входа в систему нажмите F12, чтобы открыть инструменты разработчика.",
            step5: "Откройте раскрывающийся список файлов cookie",
            step6: "Выберите www.deezer.com",
            step7:
              "Найдите файл cookie `arl` (он должен быть длиной 192 символа)",
            step8:
              "Убедитесь, что скопировано только значение, а не весь файл cookie.",
            lastStep:
              "Это ваш ARL, теперь вы можете использовать его в приложении",
          },
        },
        update: "Обновить ARL",
      },
    ...
    templates: {
      ...
      albumTracknameAvailableVariables:
        "Доступные переменные для названия трека альбома",
      playlistTracknameTemplate: "Шаблон названия трека плейлиста",
      playlistTracknameAvailableVariables:
        "Доступные переменные для названия трека плейлиста",
    },
    folders: {
      title: "Папки",
    ...
    downloads: {
      ...
      saveDownloadQueue:
        "Сохранять текущую очередь загрузок при закрытии приложения",
    },
    covers: {
      title: "Обложки",
      ...
      embeddedPNGWarning:
        "PNG официально не поддерживается в Deezer. Могут быть баги",
      imageSizeWarning:
        "Обложки с разрешением выше x1200 не поддерживаются в Deezer. Могут быть проблемы",
        ...
        clientSecretQuestion: {
          ...
          step1: {
            ...
            spotifyDevelopersDashboard:
              "информационной панели Spotify для разработчиков",
          },
          step2: {
            text: "Нажмите «Создать приложение».",
            imageAlt:
              "Кнопка «Создать приложение» на информационной панели Spotify для разработчиков",
          },
          step3: {
          ...
          step5:
            "Теперь вы можете скопировать и вставить эти результаты в соответствующие поля в настройках.",
        },
        usernameQuestion: {
