const zh_tw = {
  ...
  about: {
    ...
    subtitles: {
      ...
      contributing:
        "您想替這個專案提出貢獻嗎？您可以透過許多不同的方式來幫忙！",
      donations: "您想要捐款嗎？可以贊助我們！",
    },
    usesLibrary:
      "本程式基於 <strong>deemix</strong> 函示庫。您可以使用該函示庫來製作自己的前端。",
    thanks:
      "感謝 <strong>rtonno</strong>、<strong>uhwot</strong> 及 <strong>lollilol</strong> 對於本專案的貢獻，另外也感謝 <strong>BasCurtiz</strong> 幫忙製作本專案的圖示",
    upToDate: {
    ...
    beforeReporting:
      "在報告錯誤之前，請確保您正在運行最新版本的程式，並且要報告的內容真的是一個錯誤，而不僅是您自己遇到的問題。",
    beSure:
      "請確保您所想回報的錯誤可在其他台裝置上再現的，並且<strong>請勿</strong>回報已知錯誤。",
    duplicateReports: "請注意，重複的錯誤報告將被移除。",
    dontOpenIssues:
      "<strong>請勿</strong>透過本版本庫的 Issues 問問題，我們開 subreddit 是有原因的。",
    newUI: {
      text: "如果您精通 Python，則可以嘗試使用我們的函示庫來建立新的 UI，或者透過{repo}提出錯誤相關的 PR。",
      repo: "我們的版本庫",
    },
    acceptFeatures:
      "我也歡迎你提交新的功能－但前提是這個功能不能過於複雜－過於複雜的功能，應被新增至程式內而非函示庫內。",
    otherLanguages:
      "如果您精通其他程式語言，你也可以嘗試將 deemix 移植到其他程式語言中！",
    understandingCode:
      "你需要幫助了解本專案的程式碼嗎？只需在 Telegram 或 Reddit 上按一下 waLplanet。",
    contributeWebUI: {
      text: "如果你熟悉 Vue.js（JavaScript）、HTML 或 CSS，你也可以參與 {webui} 前端的貢獻。",
      webui: "WebUI",
    },
    itsFree:
      "記得，<strong>這是一個免費的專案</strong>，<strong>在支持開發人員之前，應該先支持你喜歡的歌手或藝人</ strong>。",
  ...
  settings: {
    ...
    covers: {
      ...
      imageSizeWarning:
        "Deezer 官方並不使用高於 x1200 的圖片，您可能會遇到問題",
    ...
    other: {
      ...
      albumVariousArtists:
        "於專輯作者欄位中保留「多位藝人 (Various Artists)」字樣",
