const ko = {
	...
	about: {
		...
		thanks:
			"이 프로젝트를 도와준 <strong>rtonno</strong>, <strong>uhwot</strong> 그리고 <strong>lollilol</strong>님과 아이콘을 만들어준 <strong>BasCurtiz</strong> 그리고 <strong>scarvimane</strong>님에게 감사를.",
		upToDate: {
		...
		beforeReporting:
			"버그 제보를 하기 전에 먼저 최신 버전의 프로그램을 실행하고 있는지, 제보하고자 하는 것이 버그이지 오로지 당신의 목적을 이루기 위한 요구가 아님을 확인해주세요.",
		beSure:
			"해당 버그가 다른 장치에서도 재현이 가능한지 확인하고 이미 제보된 버그의 경우에는 중복해서 <strong>보고하지 마세요</strong>.",
		duplicateReports:
			"중복된 버그 제보의 경우는 해당 요청을 닫을 것이니, 두 눈 뜨고 지켜보세요.",
		dontOpenIssues:
			"질문에 관한 것은 이슈를 <strong>열지 마세요</strong>, 서브레딧이 있습니다.",
		newUI: {
			text: "만약 당신이 파이썬에 능통하고 기본적인 라이브러리를 이용하여 새로운 UI를 만들었거나, 라이브러리의 버그를 수정하였다면 {repo}에 풀 리퀘스트 해주세요.",
			repo: "저장소",
		},
		acceptFeatures:
			"복잡하지 않은 기능 추가도 받습니다, 라이브러리가 아닌 앱에서 직접 구현할 수 있는 것으로요.",
		otherLanguages:
			"당신이 다른 프로그램 언어에 능통하면, 이 deemix 프로그램을 다른 프로그래밍 언어로 포팅할 수도 있습니다!",
		understandingCode:
			"코드를 이해하는 데 도움이 필요합니까? 위에 있는 waLplanet, 텔레그램, 서브레딧 링크를 누르세요.",
		contributeWebUI: {
			text: "만약 당신이 Vue.js (JavaScript), HTML 또는 CSS 개발자라면 {webui}에 도움을 주세요.",
			webui: "WebUI",
		},
		itsFree:
			"사용하기 전에 이 프로그램이 <strong>무료 프로젝트</strong>이고 개발자를 지원하기 전에 <strong>예술가들</strong>을 먼저 지원해야 함을 기억하세요.",
	...
	errors: {
		...
		ids: {
			...
			notYourPrivatePlaylist:
				"다른 사람의 비공개 재생 목록을 다운로드 할 수 없습니다.",
	...
	linkAnalyzer: {
		...
		useful:
			"접속한 국가에서는 재생할 수 없는 특정 트랙을 다운로드 하기 위해서 가능한 국가를 찾는데 유용하게 쓰입니다.",
		linkNotSupported: "해당 링크는 아직 지원하지 않습니다",
		linkNotSupportedYet:
			"해당 링크는 아직 지원하지 않습니다, 다른 링크로 시도해보세요.",
	...
	search: {
		...
		description:
			"트랙, 아티스트, 재생목록, Deezer 링크 등으로 검색할 수 있습니다!",
	...
	settings: {
		...
		covers: {
			...
			embeddedPNGWarning:
				"PNG는 Deezer에서 공식적으로 지원하지 않기 때문에 버그가 있을 수 있습니다",
			imageSizeWarning:
				"x1200 크기를 초과해서는 Deezer에서 공식적으로 사용되지 않기 때문에 문제가 생길 수 있습니다",
			coverDescriptionUTF8:
				"커버 설명을 UTF8 포맷을 이용해 저장합니다 (iTunes 커버 오류 해결)",
		},
		tags: {
			head: "저장할 태그를 선택하세요",
			title: "제목",
		...
		other: {
			...
			albumVariousArtists:
				'앨범 아티스트에 "Various Artists"(다양한 아티스트들) 유지',
