<template>
	...
	>
		...
		>
			<i
				class="p-2 text-3xl transition-all duration-500 cursor-default material-icons side_icon side_icon--theme"
			>
				{{ currentTheme }}_mode
			</i>
			<div
...
export default defineComponent({
	setup(_, ctx) {
		const activeTab = links.find(
			(link) => link.routerName === ctx.root.$route.name
		);
		...
		const updateAvailable = computed(
			() => ctx.root.$store.state.appInfo.updateAvailable
		);
