<template>
	<div id="about_tab">
		...
		<ul>
			<li v-html="$t('about.usesLibrary')" />
			<li v-html="$t('about.thanks')" />
		...
		<ul class="no-dots" style="font-size: 15px">
			...
			- Icon colors<br />
			- Created
			<a target="_blank" href="https://t.me/DeemixFix"> Releases Channel </a
			>Subscribe for updates, changelog and Builds.<br />
			- Contributions to developer accepted (all debit and credit cards):<a
				target="_blank"
				href="https://revolut.me/michelf9a"
			>
				Revolut Me </a
			><br />
			<br />
			<h3>07/08/2024</h3>
			- Restored "Spotify Username" field in Settings, Other, Spotify
			features.<br />
			- Fix for crash when fetching empty Spotify Playlists or without thumbnail
			image.<br />
			- New feature! Added the possibility to insert a list of spotify users
			instead, separated by commas or spaces.<br /><br />
			How it works:<br />
			Add a list of comma or space separated Spotify user IDs to "follow".<br />
			All the public playlists by the listed users will be displayed in the
			Favorites section. A user ID is the part that comes after
			'https://open.spotify.com/user/' and can be a set of alphanumeric
			characters or a profile name, especially for record labels. You can also
			insert your own Spotify username ID as before.<br />
			<br />
			<h3>02/08/2024</h3>
			...
			- Fixed warning "Failed to make bytecode node20-x64 for file
			dateformat.js" during build phase<br />
			- Removed server/dist/app.js and webui/public/js/bundle.js from repo,
			should not be tracked<br />
			- Changelog improvements<br />
			<br />
			<h3>22/07/2024</h3>
			- Fixed update check, it now correctly notifies for new versions on
			GDrive<br />
			<br />
			<h3>21/07/2024</h3>
			...
			- Created local dependency "deemix 3.6.15', no longer from NPM, added Uh
			Wot fix<br />
			- Created local dependency "spotify-web-api-node 5.04", no longer from
			NPM, fixed formatting and moved to ES6<br />
			- Fixed alloc buffer security issue in spotify-web-api-node, was
			outputting error in server versions<br />
			- Fixed logger errors<br />
			- Upgraded a few dependencies<br />
			- Replaced pkg with yaopkg<br />
			- updated build scripts, removed Production build for now, the DEV build
			works fine<br />
		</ul>
		...
		<ul class="no-dots">
			<li>
				<a href="https://gitlab.com/deeplydrumming/DeemixFix" target="_blank">
					💻 {{ $t("about.officialWebuiRepo") }} </a
				><br />
				<a href="https://t.me/DeemixFix" target="_blank"> 💻 Builds </a><br />
			</li>
		</ul>
		...
		<ul>
			<i18n path="about.questions.text" tag="li">
				<template #telegram>
					<a href="https://t.me/deemixcommunity" target="_blank">{{
						$t("about.questions.telegram")
					}}</a>
				</template>
			</i18n>
			<!-- <li>
